...
uint32_t sysreq_exec(uint32_t n);
uint32_t risc_time(void);
void risc_leds(uint32_t n);
...
uint8_t mem[MemBytes];
uint32_t sysarg[3], sysres;
uint32_t nargc;
char **nargv;
...
void risc_set_register(int reg, uint32_t value) {
  R[reg] = value;
  Z = value == 0;
  N = (int32_t)value < 0;
}
uint32_t le32_to_host(uint8_t *ptr) {
  return ptr[0] | (ptr[1] << 8) | (ptr[2] << 16) | (ptr[3] << 24);
}

uint32_t mem_read_word(uint32_t adr) {
  if (adr >= MemBytes - 3) {
...
uint8_t mem_read_byte(uint32_t adr) {
  if (adr >= MemBytes) {
    fprintf(stderr, "Memory read out of bounds (address %#08x)\n", adr);
    exit(1);
  }
...
void mem_write_word(uint32_t adr, uint32_t val) {
  if (adr >= MemBytes - 3) {
...
void mem_write_byte(uint32_t adr, uint32_t val) {
  if (adr >= MemBytes) {
    fprintf(stderr, "Memory read out of bounds (address %#08x)\n", adr);
    exit(1);
  }
...
void mem_check_range(uint32_t adr, uint32_t siz,
                                   const char *proc) {
...
uint32_t io_read_word(uint32_t adr) {
  switch (-adr / 4) {
...
void io_write_word(uint32_t adr, uint32_t val) {
  switch (-adr / 4) {
  /* carried over from oberon */
  case 60 / 4:
    risc_leds(val);
...
uint32_t norebo_halt(uint32_t ec, uint32_t _2, uint32_t _3) { exit(ec); }

uint32_t norebo_argc(uint32_t _1, uint32_t _2, uint32_t _3) {
  return nargc;
}

uint32_t norebo_argv(uint32_t idx, uint32_t adr, uint32_t siz) {
...
uint32_t os_getenv(uint32_t env, uint32_t adr, uint32_t siz) {
...
uint32_t norebo_trap(uint32_t trap, uint32_t name_adr, uint32_t pos) {
  char message[100];
  switch (trap) {
  case 1:
...
uint32_t files_new(uint32_t adr, uint32_t _2, uint32_t _3) {
  char name[NameLength];
...
uint32_t files_old(uint32_t adr, uint32_t _2, uint32_t _3) {
  char name[NameLength];
  if (!files_get_name(name, adr)) {
    return -1;
  }
...
uint32_t files_register(uint32_t h, uint32_t _2, uint32_t _3) {
...
uint32_t files_close(uint32_t h, uint32_t _2, uint32_t _3) {
...
uint32_t files_seek(uint32_t h, uint32_t pos, uint32_t whence) {
  files_check_handle(h, "Files.Seek");
  return fseek(files[h].f, pos, whence);
}

uint32_t files_tell(uint32_t h, uint32_t _2, uint32_t _3) {
  files_check_handle(h, "Files.Tell");
  return (uint32_t)ftell(files[h].f);
}

uint32_t files_read(uint32_t h, uint32_t adr, uint32_t siz) {
...
uint32_t files_write(uint32_t h, uint32_t adr, uint32_t siz) {
  files_check_handle(h, "Files.Write");
  mem_check_range(adr, siz, "Files.Write");
  return (uint32_t)fwrite(mem + adr, 1, siz, files[h].f);
}

uint32_t files_length(uint32_t h, uint32_t _2, uint32_t _3) {
...
uint32_t time_to_oberon(time_t t) {
  struct tm tm = {0};
  localtime_r(&t, &tm);
...
uint32_t files_date(uint32_t h, uint32_t _2, uint32_t _3) {
...
uint32_t files_delete(uint32_t adr, uint32_t _2, uint32_t _3) {
  char name[NameLength];
...
uint32_t files_purge(uint32_t h, uint32_t _2, uint32_t _3) {
  fprintf(stderr, "Files.Purge not implemented");
  exit(1);
}

uint32_t files_rename(uint32_t adr_old, uint32_t adr_new, uint32_t _3) {
...
uint32_t filedir_enumerate_begin(uint32_t _1, uint32_t _2, uint32_t _3) {
  if (dir) {
    closedir(dir);
  }
  dir = opendir(".");
...
uint32_t filedir_enumerate_next(uint32_t adr, uint32_t _2, uint32_t _3) {
...
uint32_t filedir_enumerate_end(uint32_t _1, uint32_t _2, uint32_t _3) {
  if (dir) {
    closedir(dir);
    dir = NULL;
  }
